-- Fix create_post function to use proper schema for crypt function
...
BEGIN
    ...
    INSERT INTO posts (
        ...
        public.crypt(p_author_password, public.gen_salt('bf')), v_security_key_id, 
...
-- Fix verify_security_key function as well
...
BEGIN
    ...
    LOOP
        -- 해시 비교 (crypt 함수 사용)
        IF key_record.key_hash = public.crypt(key_input, key_record.key_hash) THEN
...
-- Fix delete_post function as well
...
BEGIN
    ...
    -- 비밀번호 확인
    IF v_post_record.author_password_hash != public.crypt(p_author_password, v_post_record.author_password_hash) THEN
        RETURN FALSE;
    END IF;
